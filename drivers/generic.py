...
class GenericDriver(BaseFiscalDriver):
    ...
    def print_fiscal_invoice(self, payload):
        ...
        try:
            ...
            if receptor.get("direccion"): self._write_line(conn, f"Dir: {receptor['direccion'][:40]}")
            self._divider(conn, "-")
            for item in items:
                desc = item.get("description", "Producto")[:22]
                qty = float(item.get("quantity", 1)); price = float(item.get("unit_price", 0))
                total_item = float(item.get("subtotal", qty * price)); unit = item.get("unit", "UND")
                self._write_line(conn, desc)
                self._write_line(conn, f"  {qty:.3f} {unit} x ${price:.4f} = ${total_item:.2f}")
            self._divider(conn, "-")
            subtotal = float(payload.get("subtotal", 0)); iva = float(payload.get("monto_iva", 0))
            desc_val = float(payload.get("descuento", 0)); total = float(payload.get("total", 0))
            total_ves = float(payload.get("total_ves", 0)); tasa = float(payload.get("tasa_bcv", 0))
            if desc_val > 0:
            ...
            self._divider(conn, "=")
            self._write_line(conn, ""); self._write_line(conn, "")
            return {"success": True, "numero_control": nc, "numero_factura": nf}
        except Exception as e:
            log.error(f"Generic driver error: {e}", exc_info=True); return {"success": False, "error": str(e)}
        finally: self._close_port(conn)
    def print_test(self):
        conn = None
        try:
            conn = self._open_port()
            conn.write("*** VenPOS Bridge — Prueba de impresión OK ***\n".encode("latin-1")); conn.flush()
